...
export default class SpxHome extends Component{
  ...
  componentWillMount() {
    ...
    Icon.getImageSource('md-home', 50, normalColor).then((source) => this.setState({
      homeNormal: source
    }));
    Icon.getImageSource('md-home', 50, selectedColor).then((source) => this.setState({
      homeSelected: source
    }));
    Icon.getImageSource('md-contacts', 50, normalColor).then((source) => this.setState({
      guysNormal: source
    }));
    Icon.getImageSource('md-contacts', 50, selectedColor).then((source) => this.setState({
      guysSelected: source
    }));
    Icon.getImageSource('md-list', 50, normalColor).then((source) => this.setState({
      notificationNormal: source
    }));
    Icon.getImageSource('md-list', 50, selectedColor).then((source) => this.setState({
      notificationSelected: source
    }));
    Icon.getImageSource('md-person', 50, normalColor).then((source) => this.setState({
      meNormal: source
    }));
    Icon.getImageSource('md-person', 50, selectedColor).then((source) => this.setState({
      meSelected: source
    }));
  }
}
